    ...
    protected boolean init(InitConfig config) {
        ...
        if (isMix) {
            ...
            if (!authInfo.isSuccess()) {
                ...
                LogUtil.e(TAG, "鉴权失败 =" + errorMsg);
                return false;
            } else {
                LogUtil.i(TAG, "验证通过，离线正式授权文件存在。");
            }
        }
        ...
        if (result != 0) {
            LogUtil.e(TAG, "【error】initTts 初始化失败 + errorCode：" + result);
            return false;
        }
        // 此时可以调用 speak和synthesize方法
        LogUtil.i(TAG, "合成引擎初始化成功");
        return true;
    }
